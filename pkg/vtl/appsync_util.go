...
func (e *Engine) processUtilMathEquals(templateStr string) string {
	return appSyncUtilMathEqualsRegex.ReplaceAllStringFunc(templateStr, func(match string) string {
		...
		leftRaw := strings.TrimSpace(submatches[1])
		rightRaw := strings.TrimSpace(submatches[2])
		left := e.resolveValue(leftRaw)
		right := e.resolveValue(rightRaw)
		lf, lok := toFloat64(left)
		rf, rok := toFloat64(right)
		if lok && rok {
			return fmt.Sprintf("%v", lf == rf)
		}
