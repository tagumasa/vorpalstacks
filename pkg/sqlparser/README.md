...
## Vorpalstacks Modifications

This is a modified version of [github.com/xwb1989/sqlparser](https://github.com/xwb1989/sqlparser) for use in [Vorpalstacks](https://github.com/vorpalstacks).

### Changes

1. **Import path changed**: `github.com/xwb1989/sqlparser` → `vorpalstacks/pkg/sqlparser`

2. **Multi-dialect support**: Added support for multiple SQL dialects via `ParserOptions`:
   - `DialectMySQL` (default): backticks for identifiers, double quotes for strings
   - `DialectTimestream`: double quotes for identifiers, PostgreSQL-style cast (`::`)
   - `DialectAthena`: same as Timestream
   - `DialectPostgreSQL`: double quotes for identifiers

3. **Preprocessor pipeline**: Added `Preprocessor` for SQL transformations before parsing:
   - PostgreSQL-style cast operator (`::`) to `CAST()` conversion
   - Extensible transformation pipeline

### Usage (Vorpalstacks version)

```go
import "vorpalstacks/pkg/sqlparser"

// Standard MySQL parsing (backward compatible)
stmt, err := sqlparser.Parse("SELECT * FROM `table` WHERE a = 'abc'")

// Timestream/Athena parsing with double-quoted identifiers
opts := sqlparser.ParserOptions{Dialect: sqlparser.DialectTimestream}
pre := sqlparser.NewTimestreamPreprocessor()
sql := pre.Process(`SELECT * FROM "mydb"."table" WHERE value::double > 10`)
stmt, err := sqlparser.ParseWithOptions(sql, opts)

// Athena (same as Timestream)
opts := sqlparser.ParserOptions{Dialect: sqlparser.DialectAthena}
stmt, err := sqlparser.ParseWithOptions(sql, opts)
```
