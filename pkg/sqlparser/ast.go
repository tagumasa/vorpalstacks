...
// ObjectLiteral represents a PartiQL object literal like { 'id': 1, 'name': 'test' }.
type ObjectLiteral struct {
	Properties []*ObjectProperty
}

// Format formats the node.
func (node *ObjectLiteral) Format(buf *TrackedBuffer) {
	buf.Myprintf("{")
	for i, prop := range node.Properties {
		if i > 0 {
			buf.Myprintf(", ")
		}
		buf.Myprintf("%v: %v", prop.Key, prop.Value)
	}
	buf.Myprintf("}")
}

func (node *ObjectLiteral) walkSubtree(visit Visit) error {
	if node == nil {
		return nil
	}
	for _, prop := range node.Properties {
		if err := Walk(visit, prop); err != nil {
			return err
		}
	}
	return nil
...
func (node *ObjectLiteral) replace(from, to Expr) bool {
	for _, prop := range node.Properties {
		if replaceExprs(from, to, &prop.Value) {
			return true
		}
	}
	return false
...
// Format formats the node.
func (node *ObjectProperty) Format(buf *TrackedBuffer) {
	buf.Myprintf("%v: %v", node.Key, node.Value)
}

func (node *ObjectProperty) walkSubtree(visit Visit) error {
	if node == nil {
		return nil
	}
	return Walk(
