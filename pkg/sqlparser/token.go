...
import (
	...
	"vorpalstacks/pkg/sqlparser/dependency/bytes2"
	"vorpalstacks/pkg/sqlparser/dependency/sqltypes"
)
...
// NewStringTokenizerWithOptions creates a new Tokenizer for the
// sql string with specified parser options.
func NewStringTokenizerWithOptions(sql string, opts ParserOptions) *Tokenizer {
	buf := []byte(sql)
	return &Tokenizer{
		buf:     buf,
		bufSize: len(buf),
...
func (tkn *Tokenizer) Scan() (int, []byte) {
	...
	default:
		...
		case '!':
			...
			return int(ch), nil
		case '\'', '"':
			if ch == '"' && tkn.dialect != DialectMySQL {
				return tkn.scanLiteralIdentifierDoubleQuote()
			}
...
// scanLiteralIdentifierDoubleQuote scans a double-quoted identifier.
// This is used for Timestream, Athena, and PostgreSQL dialects.
func (tkn *Tokenizer) scanLiteralIdentifierDoubleQuote() (int, []byte) {
	buffer := &bytes2.Buffer{}
	quoteSeen := false
	for {
		if quoteSeen {
