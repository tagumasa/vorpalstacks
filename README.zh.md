...
> **注意：目前为测试版。** Vorpalstacks 正在积极开发中。已实现全部受支持的 AWS 服务（列表见[docs/services.md](docs/services.md)），Go 测试超过 3,000 项（涵盖 SDK、跨服务集成与 WebSocket 套件，另有 Python 631 项、TypeScript 2028 项、C# 2019 项），但并未完全覆盖所有边界情况和 AWS 行为。可能存在破坏性变更。欢迎提交 Bug 报告和贡献。
...
这种架构使核心操作达到亚毫秒级延迟，可以在 CI/CD 流水线中直接运行大量 API 测试（Go 超过 3,000 项，涵盖 SDK、集成与 WS 套件；Python 631 项、TypeScript 2028 项、C# 2019 项），无需容器化开销。
