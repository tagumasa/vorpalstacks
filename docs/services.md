...
**SDK Tests**: 3,483 passed, 0 failed (3,416 SDK + 50 integration + 17 WebSocket)
...
| CloudFront | Broad | enabled | Origin proxy with cache behaviours, TTL edge cache, invalidation, CNAME aliases, continuous deployment policies, viewer TLS serving (SNI per distribution from the attached ACM/IAM certificate), and ViewerProtocolPolicy enforcement |
...
| WAFv2 | Broad | enabled | GeoMatch/AsnMatch resolve through an embedded table derived from RIR delegated-extended allocation data and a RouteViews routing-table snapshot (regenerated by internal/tools/wafgeogen), so country results follow registry allocations rather than a commercial geolocation database and can differ from AWS WAF for reassigned addresses; CAPTCHA and Challenge run on a platform token — the aws-waf-token cookie is HMAC-signed locally, text/html clients receive a JavaScript interstitial whose proof-of-work challenge is exchanged at the reserved POST /awswaf/token endpoint, with the documented 405/202 interrupts and immunity times; Monetize validates its configuration and interrupts with the 402 price manifest, but payment settlement requires blockchain network access and is not verified; managed rule groups are served from a local catalog of the fifteen documented groups — the nine signature groups (63 rules) evaluate against local statements derived from the published rule descriptions (AWS publishes no exact match patterns, so each local statement covers the documented examples plus the canonical public signatures of the same threat class), while the six data-dependent groups (IP reputation, anonymous IP, Bot Control, ATP, ACFP, Anti-DDoS; 73 rules) and the Known Bad Inputs ReactJS RCE rule never match locally because their inputs (threat-intelligence feeds, device fingerprints, ML models, unpublished advisory patterns) exist only inside AWS; the HeaderOrder component preserves the wire order on HTTP/1.1 connections and falls back to the header map's order on HTTP/2 connections |
