...
| SNS | Broad | No SMS, email/email-json, mobile push (application protocol), or Firehose-protocol subscription delivery; no FIFO message archiving/replay, SSE-KMS encryption, X-Ray active tracing, or fetchable signing certificates |
...
- **SNS — delivery retry ladder**: the HTTP/S delivery policy is implemented in full — the four-phase healthyRetryPolicy ladder (no-delay, minimum-delay, backoff with the four documented backoff functions, maximum-delay), the throttle policy's maxReceivesPerSecond pacing, and the request policy's headerContentType, resolvable at both topic and subscription level with disableSubscriptionOverrides and a synthesised EffectiveDeliveryPolicy on both read planes; 5XX and 429 responses and transport failures retry, every other status is a permanent failure. SQS and Lambda deliveries — which AWS retries up to 100,015 times over 23 days under a fixed policy — attempt once and route failures straight to the subscription's dead-letter queue: the platform's invokers are in-process, and weeks-long in-process retry holds have no observable AWS-parity value on this substrate; in-flight retries also do not survive a server restart.

- **SNS — MaximumMessageSize ceiling**: the topic attribute is honoured per topic (validated, echoed when set, and enforced at publish against the topic's value), but values above 256 KiB are rejected rather than accepted: the platform's SQS and Lambda invocations cap every payload at 256 KiB, so the documented 1 MiB mode (which additionally restricts the topic to 100 or fewer SQS/Firehose/Lambda subscriptions) cannot be honoured on this transport.

- **SNS — FIFO archive and SSE attributes**: ArchivePolicy and ReplayPolicy are accepted, stored and echoed while message archiving/replay itself is absent (BeginningArchiveTime is never synthesised); KmsMasterKeyId is accepted, stored and echoed while server-side encryption is not implemented. Both planes stay honest by having no behaviour claim to honour on this platform.

- **SNS — TracingConfig and message-signing exclusions**: the TracingConfig topic attribute is accepted, stored and echoed as a pass-through while the platform runs no X-Ray active tracing. Notifications and confirmations are signed with the platform's own RSA key (SignatureVersion 1 over SHA1, 2 over SHA256, on the documented StringToSign), but the envelope's SigningCertURL names the AWS-account-owned sns.<region>.amazonaws.com domain the platform cannot serve — signature verification requires distributing the platform certificate out of band.

- **SNS — application protocol refused at Subscribe**: mobile push delivery requires the external push services (APNs, FCM and siblings) this platform never provides, so Subscribe rejects the application protocol outright instead of creating an auto-confirmed subscription that would silently drop every publish; the platform-application and endpoint planes (CreatePlatformApplication, CreatePlatformEndpoint and siblings) remain implemented for storage and ARN identity.

- **SNS — Firehose subscription protocol**: Subscribe accepts the firehose protocol and stores the subscription as pending confirmation; no delivery handler exists until the platform Firehose service does, so every publish to a firehose subscription routes to the dead-letter path. This is the same accept-and-pend carryover the Cognito and EventBridge Firehose destinations record: delivery activates together with the Firehose service.

- **SNS — enforced quotas**: the documented per-account topic quotas (100,000 standard / 1,000 FIFO), per-topic subscription quotas (12,500,000 standard / 100 FIFO) and per-topic filter-policy quota (200) are enforced with their model-declared errors (TopicLimitExceeded, SubscriptionLimitExceeded, FilterPolicyLimitExceeded); the account-scope filter-policy quota (10,000 per account) is not enforced because it aggregates across Regions and the platform keeps no cross-Region account view.

- **SNS — subscription and permission semantics**: Subscribe is idempotent on the subscription natural key (topic, protocol, endpoint, owner) — a repeated Subscribe returns the live subscription's ARN instead of accumulating duplicates; FIFO topics refuse every customer-managed endpoint protocol (email, email-json, sms, application, http, https) at Subscribe, keeping SQS, Lambda and Firehose; AddPermission refuses a duplicate Label instead of overwriting the existing statement.
