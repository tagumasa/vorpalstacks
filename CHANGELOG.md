...
- **Scheduler** — AWS cron L/W/# wildcard support via forward-scanning evaluator with OR semantics for day-of-month/day-of-week; persisted retry records with at-least-once delivery; RetryPolicy enforcement (MaximumRetryAttempts, MaximumEventAgeInSeconds); exponential backoff with crypto/rand jitter; DeadLetterConfig routing to SQS or SNS; ActionAfterCompletion=DELETE deferred to lifecycle completion; ScheduleExpressionTimezone evaluation; at() schedules ignore StartDate/EndDate; Kinesis, SQS, and EventBridge target parameters; rate-expression validation; FlexibleTimeWindow validation; Name/GroupName regex; UpdateSchedule PUT semantics.
