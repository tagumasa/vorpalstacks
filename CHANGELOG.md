...
- **Neptune Data**: loader completion preserves concurrent CancelLoaderJob status; loadNTriples returns cancelled status consistent with other loaders; ExecuteFastReset token map uses type guards and purges expired entries; query status and list responses normalise nil slices to empty arrays per AWS-spec; removed dead WebSocket session-cleanup constants.
