...
- **Dispatcher: removed resilience wrapper** — HTTP request dispatcher no longer wraps handler invocations in the resilience layer. `resilience.ServiceResilienceConfig` removed from `NewDispatcher` and `NewServer`.
...
- **Athena: ListWorkGroups pagination** — Was fetching up to 10,000 rows into memory. Now uses store-level `Marker`/`MaxItems` with default page size 50.

- **Lambda: SQS event source batching, multi-region containers, Python runtime** — SQS polling loops to honor `BatchSize` up to 10,000. Container names include region. `copyCodeToContainer` selects `index.py` for Python. ESM poller removed redundant `accountID` field.

- **EventBridge: wildcard suffix matching and InputTransformer value serialization** — `matchWildcardPattern` uses `strings.HasSuffix` when no `*` present. `applyInputTransformer` JSON-serializes non-string values.

- **Store: hardcoded pagination limits replaced with generic utilities** — Added `ListMatching`, `ListMatchingProto`, `ForEachAll`, `ForEachAllProto` to `common/base_store.go`. 17 call sites migrated. Secrets Manager `ListSecrets` evaluates filters inside the Pebble iterator. Cognito IDP list operations use store-level pagination. CloudWatch operations migrated to `common.List[T]`. `ListMetrics` default page size set to 500.

- **IAM: `GetAccountAuthorizationDetails` pagination** — Was truncated at 1000; now supports `Marker`/`MaxItems` across User/Group/Role/Policy sections with cross-section continuation.
...
- **S3: PebbleDB key delimiter changed to null byte** — Internal storage key separator changed from `#` to `\x00` to eliminate collision risk with S3 object keys containing `#`. Non-versioned objects use `bucket\x00key\x00null` consistently, replacing the previous `bucket#key` / `bucket#key#null` dual-path fallback. `ListParts` replaced redundant `countSkipped` with inline counter.
...
- **DynamoDB: PebbleDB key delimiter changed to null byte** — Internal storage key separator changed from `#` to `\x00` to eliminate collision risk with DynamoDB table/key names containing `#`. Affects item, GSI index, and LSI index buckets.

- **DynamoDB: key attribute update rejection** — `UpdateItem` and `TransactWriteItems` Update now reject attempts to modify partition key or sort key attributes via `UpdateExpression` or `AttributeUpdates`, matching AWS behavior (`ValidationException: Cannot update attribute … This attribute is part of the key schema`).

- **DynamoDB: item size calculation** — Number size now uses AWS formula (significant digit pairs → 1 byte, minimum 1 byte) instead of raw string length. Map and List types include 3-byte overhead matching AWS accounting.

- **DynamoDB: storage-level scan pagination** — New `ScanWithOptions` with `Limit`/`Marker` pushed to the PebbleDB scan layer, avoiding full-table loads into memory. `Query` and `Scan` operations use store-level limits where possible.
