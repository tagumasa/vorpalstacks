...
- **Neptune overhauled**: Events migrated from `BaseStore` JSON to `ProtoStore[Event]` with dedicated proto message; `clusterToResponseMap` replaces JSON round-trip with explicit field mapping; `buildRestoredCluster` shared by restore paths with expanded source attribute inheritance; `reparentClusterResources` migrates tags and instance refs on cluster rename; `buildParameterItems`/`applyParameterModifications` deduplicate parameter group handlers; `DescribeDBEngineVersions` collapsed to struct+loop; `paginateItems` delegates to `pagination.PaginateSlice`; `ProtoStore` list methods skip corrupted entries with warning; `snapshot_type` added to DBClusterSnapshot proto
- **NeptuneGraph overhauled**: Periodic expired-task cleanup via `regionCleanups` sync.Map with 24h production interval (5m/30m in test mode), replacing fire-and-forget goroutines; `store()` triggers cleanup on region-first access and periodically thereafter
...
- **SNS overhauled**: Message attribute parsing consolidated into `parseMessageAttributes` helper, eliminating Publish/PublishBatch duplication; subscription attribute parsing merged into generic `parseAttributes`; Lambda delivery payload construction unified via `buildNotificationPayload`; `NewTopicNotFoundException` replaced with package-level `ErrTopicNotFound` variable; `TopicListResult`/`SubscriptionListResult` unused types removed; `GetTopicAttributes` now returns `KmsMasterKeyId`, `EffectiveDeliveryPolicy`, `CreatedDate`, and `LastModifiedTime`; `SetTopicAttributes` syncs attributes back to struct fields; fixed `ListEndpointsByPlatformApplication` including the marker item in results, and signature string composition (`UnsubscribeURL` removed, trailing newline on `Type` corrected)
