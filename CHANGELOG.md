...
- **Cognito IDP** — Per-client token validity (AccessTokenValidity, IDTokenValidity in minutes, RefreshTokenValidity in days from UserPoolClient config, replacing hardcoded 60/60/30 defaults); ID token persistence (stored alongside access and refresh tokens for validation and revocation); SignUp enforcement of AdminCreateUserConfig.AllowAdminCreateUserOnly (rejects self-registration when admin-only creation is enabled); GenerateSecret=false suppression on CreateUserPoolClient (JSON bool and Query string protocols); AllowedOAuthFlowsUserPoolClient parse and always-on response; PreventUserExistenceErrors as string on UserPoolClient (was bool on PasswordPolicy); PreSignUp Lambda trigger now receives ValidationData and ClientMetadata from request; identity provider AttributeMapping and IdpIdentifiers on Create/Update/Describe; DeleteUserPool cascade deletion of resource servers, identity providers, and domains; device tracking (ConfirmDevice with SRP salt-verifier, ForgetDevice with existence check, ListDevices with PaginationToken, UpdateDeviceStatus remembered/not_remembered validation, admin device operations); per-user MFA preferences (AdminSetUserMFAPreference, SetUserMFAPreference for SMS/Email/SoftwareToken, GetUserAuthFactors with preferred factor detection, SetUserSettings); token auth (RevokeToken, GetTokensFromRefreshToken without new refresh token, per-attribute verification codes with expiry, VerifyUserAttribute with constant-time comparison and verified flag, ResendConfirmationCode with pool AutoVerifiedAttributes-aware delivery); auth event audit (AdminListUserAuthEvents, admin/user event feedback); client secret lifecycle (Add/Delete/ListUserPoolClientSecrets); log delivery configuration (Get/SetLogDeliveryConfiguration); risk configuration (SetRiskConfiguration with compromised-credentials and account-takeover actions, notify email templates, and IP range exceptions); UI customisation (Get/SetUICustomization); user import jobs (Create/Describe/List/Start/Stop with pre-signed CSV URL); WebAuthn credentials (Start/CompleteRegistration, List/Delete); managed login branding (Create/Describe/DescribeByClient/Update/Delete); terms (Create/Describe/List/Update/Delete); cross-region replicas (Create/List/Delete/UpdateUserPoolReplica); miscellaneous operations (AddCustomAttributes, GetIdentityProviderByIdentifier, GetSigningCertificate, AdminDisableProviderForUser, AdminLinkProviderForUser); thread-safe provisioned limits (Get/UpdateProvisionedLimit with sync.RWMutex).
...
- **Cognito IDP**: AdminDeleteUser now deletes tokens before the user record (was deleting user first, leaving valid tokens if token deletion failed).
