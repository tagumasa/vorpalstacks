...
- **RDS** — DBInstance and DBClusterSnapshot fields round-tripped through Describe and snapshot creation; Filter support across Describe* handlers; DescribeEvents full filter coverage; final snapshot on Delete; Engine required on Create; CreateDBCluster opens per-cluster engine; engine-family-aware default parameters; AWS-spec identifier and Engine/EngineVersion whitelist validation; Neptune handler dispatches MySQL engine; **Data API**: AWS-spec exception taxonomy; parameter validation with SQL-injection guards; TypeHint-aware SQL substitution; response classification by engine schema; DML RowsAffected and InsertID; ResultSetOptions honoured; 1 MiB response cap; 45-second statement timeout with ContinueAfterTimeout; AWS-spec transaction timeouts (idle 3 min, max-life 24 h); SecretResolver wired to Secrets Manager.
