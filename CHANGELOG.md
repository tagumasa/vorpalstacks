...
- **Neptune**: AddRole/RemoveRole raise AWS-spec faults; snapshot operations use MissingParameter error type; DeleteDBCluster persists deleting status before final snapshot; EngineVersion validated with DefaultEngineVersion fallback; DescribeDBSnapshots/DescribeDBSubnetGroups paginate with Marker/MaxRecords; DescribeOrderableDBInstanceOptions uses DefaultEngineVersion; PromoteReadReplicaDBCluster removes from global cluster members; RestoreDBClusterFromSnapshot validates cluster ID and engine.
