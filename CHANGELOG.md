...
- **ACM** — SubjectAlternativeNames embedded in x509 certificate template; KeyAlgorithm support (RSA 2048/3072/4096, EC prime256v1/secp384r1/secp521r1); SearchCertificates with FilterStatement, SortBy/SortOrder, and pagination; generic tag API; domain name and ValidationMethod validation; RevocationReason enum validation; ImportCertificate PrivateKey requirement; ExportCertificate passphrase validation; tag quota (max 50) and SAN count limit (max 100); per-domain validation options with DNS CNAME records; ListCertificates advanced filtering (keyTypes, keyUsage, managedBy, CertificateKeyPairOrigins); CertificateKeyPairOrigin and ManagedBy fields; CertificateOptions.Export configurable; CertificateAuthorityArn on RequestCertificate; ResendValidationEmail Domain/ValidationDomain handling; ARN format validation.
- **API Gateway** — MethodSettings and CanarySettings patch on UpdateStage; stage throttling enforcement (token-bucket rate limiter, wildcard settings merge); HTTP and DynamoDB non-proxy integration pipeline (templates, passthrough behaviour); binary media types with Lambda proxy base64; integration content handling and stage variable substitution; per-integration timeout; TlsConfig and inline IntegrationResponses; method AuthorizationScopes; RestApi DisableExecuteApiEndpoint, SecurityPolicy, cloneFrom; DomainName MutualTlsAuthentication and RoutingMode; Stage CanarySettings on CreateDeployment; ARN-based tag dispatch; JSON-Pointer sub-path patch on UpdateMethod/Integration/Authorizer/UsagePlan; authorizer type validation.
- **AppSync** — PER_RESOLVER_CACHING with SHA-256 key and TTL expiry; API key expiry validation and count limit; CachingConfig TTL validation; combined API quota; tags in GraphQL API, Event API, and Channel Namespace responses; AppSyncRuntime validation; SourceApiAssociationConfig with MergeType; GraphQL API_KEY authentication with expiry check; ErrorType in GraphQL errors; real introspection JSON from parsed SDL; includeDirectives honoured; protocol-specific access denied error codes in dispatcher.
- **Athena** — BytesScannedCutoffPerQuery enforcement with CANCELLED state; StartQueryExecution rejects disabled workgroups; ResultConfiguration ACLConfiguration, EncryptionConfiguration, ExpectedBucketOwner, and Remove* fields in UpdateWorkGroup; WorkGroup configuration (AdditionalConfiguration, ExecutionRole, CustomerContentEncryptionConfiguration, EnableMinimumEncryptionConfiguration); capacity reservation and assignment APIs; ListApplicationDPUSizes; UNION/UNION ALL query execution; DDL column attributes (NOT NULL, DEFAULT, COMMENT); qualified table name support; SELECT column type inference from schema; DeleteCapacityReservation API; GetDataCatalog returns default AwsDataCatalog as GLUE type.
...
- **SESv2** — BYODKIM on CreateEmailIdentity and PutEmailIdentityDkimSigningAttributes; GetEmailIdentity returns Tags and Policies inline; ListSuppressedDestinations and ListContacts filters; configuration set expansion (SuppressionOptions, VdmOptions, ArchivingOptions); GetConfigurationSet returns Tags inline; CreateDedicatedIpPool Tags; PutAccountVdmAttributes nested parsing; ProductionAccessEnabled nullable semantics; DKIM attributes expanded; contact-list topic field validation.
...
- **STS** — MFA verification in AssumeRole and GetSessionToken (RFC 6238 TOTP with condition key injection); session tags with role-chaining transitive propagation and privilege-escalation prevention; AssumeRole validation (ExternalId, RoleSessionName, SourceIdentity, session policy); AssumeRoot validation (TargetPrincipal, TaskPolicyArn whitelist); AssumeRoleWithSAML and AssumeRoleWithWebIdentity validation; root-user duration cap; GetWebIdentityToken JWT signing (RS256/ES384); PackedPolicySize includes session tags.
...
- **AppSync**: `$util.appendError` no longer halts resolver execution (now returns partial data alongside collected errors); DomainName HostedZoneId returns CloudFront zone ID; EventBridge event source corrected; ApiCache initial status CREATING with async AVAILABLE; StartSchemaMerge returns MERGE_IN_PROGRESS with async completion; EvaluateCode/EvaluateMappingTemplate return BadRequestException on missing inputs; UpdateGraphqlApi/UpdateApi required fields enforced; UpdateApiCache TTL distinguishes absent from 0; schema creation status persisted with retry backoff.
- **Athena**: StopQueryExecution returns InvalidRequestException for non-cancellable state; capacity reservation not-found returns InvalidRequestException; S3 result output restricted to DML statements; getTableData S3 failures propagate as errors; DataScannedInBytes computed from actual cell lengths; GetQueryRuntimeStatistics computed from actual results; CREATE TABLE no longer eagerly creates empty data; LOCATION parsing uses word-boundary match; qualified table names preserved in DROP TABLE.
...
- **IAM**: DeleteGroup/DeleteUser now return conflict errors for dependent resources (was silently cascade-deleting); RenameUser restructured to two-phase migration with rollback; error HTTP status codes aligned to Smithy (LimitExceeded 409, ValidationError→InvalidInput); credential report password fields populated from login profile and policy (were hardcoded N/A); MFA Base32StringSeed properly base32-decoded then base64-encoded (was double-encoding); TOTP consecutive validation requires adjacent steps; GetServiceLinkedRoleDeletionStatus returns NoSuchEntityException for unknown tasks; cascade-delete propagates errors (was silently swallowed).
...
- **Timestream Write**: WriteRecords validates batch size (1–100) and merges CommonAttributes per AWS-spec; retention properties enforce required traits and ranges; UpdateDatabase rejects empty KmsKeyId; ResumeBatchLoadTask uses atomic compare-and-swap; list operations honour pagination caps and return NextToken; RecordsIngested.Total reports actual count; parseTimestamp uses strict parsing and rejects negative epochs; empty MeasureValueType defaults to DOUBLE; unused store-interface declarations deleted.
