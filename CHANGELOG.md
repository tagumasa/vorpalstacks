...
- **Lambda** — Asynchronous invocation retry with EventInvokeConfig; async destination delivery (OnSuccess/OnFailure to SQS/SNS/Lambda/EventBridge); event source mapping filter criteria (prefix, suffix, numeric range, exists, anything-but); alias weighted routing; layer version policy APIs; reserved concurrency enforcement (atomic inflight counter); LogType=Tail; provisioned concurrency pre-warm; orphaned container cleanup on startup; SubnetUsageChecker and SecurityGroupUsageChecker for EC2 dependency checks.
