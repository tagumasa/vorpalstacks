...
- **Athena** — DeleteCapacityReservation API; BytesScannedCutoffPerQuery enforcement (query cancelled post-execution when scanned data exceeds workgroup limit, results deleted, CANCELLED state with StateChangeReason); StartQueryExecution rejects disabled workgroups (InvalidRequestException); ResultConfiguration ACLConfiguration with S3AclOption enum validation (BUCKET_OWNER_FULL_CONTROL only, wire-format key `S3AclOption` matching AWS SDK); ResultConfiguration EncryptionConfiguration and ExpectedBucketOwner in UpdateWorkGroup; ResultConfigurationUpdates Remove* fields (RemoveOutputLocation, RemoveEncryptionConfiguration, RemoveExpectedBucketOwner, RemoveAclConfiguration); WorkGroup configuration AdditionalConfiguration, ExecutionRole, CustomerContentEncryptionConfiguration (KmsKey), and EnableMinimumEncryptionConfiguration in CreateWorkGroup parse, response serialization, and UpdateWorkGroup; ConfigurationUpdates RemoveBytesScannedCutoffPerQuery, PublishCloudWatchMetricsEnabled, EngineVersion, and RemoveCustomerContentEncryptionConfiguration.
- **CloudFront** — PublicKey CRUD (Create/Get/GetConfig/Update/Delete/List with PublicKeyStore, ARN builder, CallerReference auto-generation, name uniqueness check, EncodedKey immutability enforcement via CannotChangeImmutablePublicKeyFields); KeyGroup CRUD (Create/Get/GetConfig/Update/Delete/List with KeyGroupStore, ARN builder, replacing empty stub; TooManyPublicKeysInKeyGroup max-5 quota, KeyGroupAlreadyExists name collision on update, ResourceInUse distribution reference check on delete via isKeyGroupReferenced scanning DefaultCacheBehavior and CacheBehaviors TrustedKeyGroups); If-Match ETag enforcement (InvalidIfMatchVersion when header absent on Update/DeleteKeyGroup and Update/DeletePublicKey, PreconditionFailed on mismatch); PublicKeyInUse check on DeletePublicKey via isPublicKeyReferenced scanning key group Items; distribution config round-trip expansion (CustomErrorResponses Items, GeoRestriction Locations, Origin CustomHeaders Items and OriginShield, ForwardedValues Cookies WhitelistedNames/Headers/QueryStringCacheKeys Items, LambdaFunctionAssociations and FunctionAssociations Items with EventType/LambdaFunctionARN/FunctionARN, TrustedSigners and TrustedKeyGroups Items with AwsAccountNumber/KeyGroupId XML elements; parseStringItemList helper handling both JSON array and XML nested-map forms; parseForwardedValues and formatForwardedValues extracted as shared helpers).
...
- **CloudWatch Logs** — LogGroup KmsKeyId and DeletionProtectionEnabled fields (proto 11–12) with DeleteLogGroup enforcement and CreateLogGroup validation (LogGroupClass STANDARD/INFREQUENT_ACCESS, KmsKeyId `arn:aws:kms:` prefix); kmsKeyId/logGroupClass/deletionProtectionEnabled surfaced in DescribeLogGroups; AWS-spec validation on PutRetentionPolicy (retention whitelist), PutSubscriptionFilter (distribution enum), PutLogEvents (MaxChunkSize=10000), and PutDestination Tags; chunk file format V2 with per-entry ingestion timestamp (`chunk.Ingestible` interface; V1 reader retained); `request.GetBoolParamDefault` helper; GetLogEvents_ForwardPagination SDK test; integration tests wrapped in 3-minute per-test `runIntegWithTimeout`; `RunServicesParallel` phase routing uses CategoryIntegration enum; `PutLogEvents` atomic transactional commit of LogGroup + LogStream + chunk indexes (`prepareChunkFlush` + `updateLogGroupStreamAndChunks`, orphaned chunk file cleanup on transaction failure); shared forward/backward pagination boundary helper (`pagination.go`).
...
- **Cognito IDP** — Per-client token validity (AccessTokenValidity, IDTokenValidity in minutes, RefreshTokenValidity in days from UserPoolClient config, replacing hardcoded 60/60/30 defaults); ID token persistence (stored alongside access and refresh tokens for validation and revocation); SignUp enforcement of AdminCreateUserConfig.AllowAdminCreateUserOnly (rejects self-registration when admin-only creation is enabled); GenerateSecret=false suppression on CreateUserPoolClient (JSON bool and Query string protocols); AllowedOAuthFlowsUserPoolClient parse and always-on response; PreventUserExistenceErrors as string on UserPoolClient (was bool on PasswordPolicy); PreSignUp Lambda trigger now receives ValidationData and ClientMetadata from request; identity provider AttributeMapping and IdpIdentifiers on Create/Update/Describe; DeleteUserPool cascade deletion of resource servers, identity providers, and domains; auto-verified attribute marking on ConfirmSignUp, AdminConfirmSignUp, and auto-confirmed AdminCreateUser (email, phone_number flagged per pool AutoVerifiedAttributes); device tracking (ConfirmDevice with SRP salt-verifier, ForgetDevice with existence check, ListDevices with PaginationToken, UpdateDeviceStatus remembered/not_remembered validation, admin device operations); per-user MFA preferences (AdminSetUserMFAPreference, SetUserMFAPreference for SMS/Email/SoftwareToken with single-preferred enforcement and verified-attribute prerequisite, GetUserAuthFactors with preferred factor detection, SetUserSettings); token auth (RevokeToken, GetTokensFromRefreshToken without new refresh token, per-attribute verification codes with expiry and non-existent-attribute rejection, VerifyUserAttribute with constant-time comparison and verified flag, ResendConfirmationCode with pool AutoVerifiedAttributes-aware delivery); auth event audit (AdminListUserAuthEvents, admin/user event feedback with FeedbackValue validation); client secret lifecycle (Add/Delete/ListUserPoolClientSecrets with server-side pagination); log delivery configuration (Get/SetLogDeliveryConfiguration with LogLevel/EventSource validation); risk configuration (SetRiskConfiguration with enum-validated compromised-credentials and account-takeover actions, notify email templates, and IP range exceptions; DescribeRiskConfiguration includes all configured action levels); UI customisation (Get/SetUICustomization with CSSVersion increment on CSS change only); user import jobs (Create/Describe/List/Start/Stop with pre-signed CSV URL and PaginationToken); WebAuthn credentials (Start/CompleteRegistration, List/Delete with NextToken pagination); managed login branding (Create/Describe/DescribeByClient/Update/Delete with scan-based client lookup); terms (Create/Describe/List/Update/Delete with NextToken pagination); cross-region replicas (Create/List/Delete/UpdateUserPoolReplica with NextToken pagination); miscellaneous operations (AddCustomAttributes, GetIdentityProviderByIdentifier, GetSigningCertificate, AdminDisableProviderForUser with provider-name and attribute-value lookup, AdminLinkProviderForUser); thread-safe provisioned limits (Get/UpdateProvisionedLimit with sync.RWMutex).
...
- **IoT Events** — SimpleRule alarm evaluator engine (`alarm_evaluator.go`: `$input.X.attr` reference resolution, 6 comparison operators, threshold as numeric literal or `$input` reference with automatic resolution, RLock snapshot → per-transition write-lock reacquisition for safe interleaving with AcknowledgeAlarm); alarm state machine Pebble persistence (dedicated `iot-alarm-state` bucket, `loadFromStore` rebuild on startup, `persist` on each transition); `hydrateAlarmModels` bulk-loads existing alarm models into the state machine on init; BatchDeleteDetector/UpdateDetector wired with error entries for missing fields; detector model `evaluationMethod` (BATCH/SERIAL) validation.
...
- **STS** — MFA verification in AssumeRole and GetSessionToken (RFC 6238 TOTP via IAM virtual MFA devices with ±1 window, trust policy condition key injection for sts:ExternalId and aws:MultiFactorAuthPresent/aws:MultiFactorAuthAge, MFA state persisted in session); session tags in AssumeRole and GetFederationToken (Tags and TransitiveTagKeys parsing with duplicate key rejection and Smithy tagKeyType/tagValueType validation, role-chaining transitive tag propagation with privilege-escalation prevention); AssumeRole parameter validation (ExternalId pattern and 2–1224 length, RoleSessionName and SourceIdentity pattern and 2–64 length with reserved "aws:" prefix rejected, session policy JSON validation, SerialNumber/TokenCode MFA format checks); AssumeRoot parameter validation (TargetPrincipal 12–2048, TaskPolicyArn root-task managed policy whitelist, RootDurationSeconds 0–900 default 900); AssumeRoleWithSAML RoleSessionName and session policy validation; AssumeRoleWithWebIdentity RoleSessionName, SourceIdentity, session policy, token length (4–20000), and providerId ARN-or-URL acceptance; GetFederationToken and GetSessionToken root-user 3600-second duration cap, userNameType validation (2–32, pattern), and PackedPolicySize via shared computePackedPolicySize; GetWebIdentityToken JWT signing (RS256/ES384 with lazy key pair generation, Audience list max 10, SigningAlgorithm validation, DurationSeconds 60–3600 default 300); DecodeAuthorizationMessage length check (≤10240); GetAccessKeyInfo length check (16–128) and Status field; PackedPolicySize computed once and includes session tags; CreateSessionParams struct replacing positional parameters.
...
- **CloudFront**: distribution config response now serializes LambdaFunctionAssociations, FunctionAssociations, TrustedSigners Items, TrustedKeyGroups Items, CustomErrorResponses Items, GeoRestriction Items, Origin CustomHeaders Items, and OriginShield (were hardcoded to empty defaults or Quantity:0, breaking round-trip); ForwardedValues now serializes Cookies WhitelistedNames, Headers, and QueryStringCacheKeys Items (was QueryString and Cookies.Forward only); ListCachePolicies and ListOriginRequestPolicies now return full config structs including ParametersInCacheKeyAndForwardedToOrigin (was Name/Comment/TTL only); ListResponseHeadersPolicies now returns full config via buildResponseHeadersPolicyConfigResponse (was Name/Comment only).
...
- **CloudWatch**: ComparisonOperator name corrected from `LessThanOrEqualToLowerOrGreaterThanUpperThreshold` to `LessThanLowerOrGreaterThanUpperThreshold` (AWS spec); GetMetricData, dashboard widget, and metric data result statistic matching now case-insensitive (was exact-match, rejecting lowercase stat names from some SDKs); widget unknown statistics now resolve from ExtendedStats instead of silently defaulting to Average; integration alarm tests now use Period 60 with period-aligned timestamps (was Period 1, rejected by validation added in previous batch; missing Timestamp caused empty evaluation windows); alarm mute rule status recomputed at read time in GetAlarmMuteRule and ListAlarmMuteRules so SCHEDULED→ACTIVE→EXPIRED transitions are always accurate without a background reaper; ErrAlarmMuteRuleNotFound sentinel error for errors.Is discrimination; ListAlarmMuteRules summary response reduced to 5 fields; anomaly band computation extracted to shared prepareAnomalyBand; anomaly detector response emits SingleMetricAnomalyDetector nested object and StateValue (was flat State — AnomalyDetectorType removed as non-existent in AWS API spec); PutLogAlarm action arrays now accept lowercase parameter aliases.
- **CloudWatch Logs**: FilterLogEvents `startFromHead` default corrected to `true` per AWS spec (was `false`, returning newest-first by default); event reversal moved inside `store.FilterLogEvents` before pagination, matching `GetLogEvents` (was per-slice, non-monotonic across pages); `startFromHead=false` rejected when `startTime` precedes 2024-01-01 UTC; `logStreamNames` + `logStreamNamePrefix` mutual exclusion; `DescribeLogStreams` `orderBy=LastEventTime` now sorts all streams before paginating with offset-based tokens (was single-page sort; name-keyed markers unstable when LastEventTime shifts), honours `descending` for `orderBy=LogStreamName`, and rejects `logStreamNamePrefix`; `fetchAllLogStreams` walks marker chain (was hard 10k cap); `ListLogStreams` prefix passed through `escapePath` (was raw concat on names containing `/`, `:`, `\`); `GetLogEvents` tokens pure numeric offsets (was `f-N`/`b-N`, unparseable by the store's `%d` parser); per-event `ingestionTime` persisted via chunk V2 (was unconditionally `time.Now()` at read); `PutLogEvents` metric/subscription processing moved to recovered background goroutine (was blocking response) and pre-flushes active chunk before MaxChunkSize overflow; `MetricFilterCount` atomic under `filterMutex` in store layer (was non-atomic read-modify-write in handler); orphan chunk file removed when index PutProto fails after flushChunk; `PurgeAllExpiredChunks` paginates all groups via marker loop (was 10k cap); `incrementToken` uses `strconv.Atoi` with error propagation (was silent fallback masking corruption); `FilterLogEvents` entered an infinite pagination loop when the nextToken offset landed exactly on a data boundary, returning the same token indefinitely (now performs Forward/Backward offset boundary check and returns an empty token when the boundary is reached); metric filter rollback errors now logged via `logs.Warn` (was `_ = s.Delete(key)` silently discarding the rollback failure); chunk test `TestZeroIngestionTimeUsesV1Path` renamed to `TestZeroIngestionTimeRoundTrip` to match its actual behaviour, with new `TestIngestionTimeRoundTrip` exercising the V2 round-trip across zstd and gzip encodings.
...
- **Route 53**: recordSetToResponse now serializes MultiValueAnswer and TrafficPolicyInstanceId (were parsed on input but omitted from responses, breaking round-trip); Weight display condition refined to show only when SetIdentifier present without other routing policies (was `Weight > 0`, omitting Weight=0 for weighted records and spuriously showing Weight=0 for latency/failover/geo/multivalue records).
- **S3**: versioning cache stale reads after status change (ristretto `Wait()` added to `VersioningCache.Set` for immediate visibility).
- **Scheduler**: schedules configured for multiple regions fired once per region because `lastFired` keys omitted the region, causing all regions to share a single dedup slot (now uses `lastFiredKey(region, group, name)` with `allActiveKeys` collected across all regions before a single cleanup sweep); rate-expression schedules with a future `StartDate` fired immediately (now guarded by `now.Before(*StartDate)` at the top of `shouldExecute`); `lastFired.Store` was reserved before goroutine spawn, so a panic or error during execution left a "fired but never ran" state leak (now `shouldExecute` is a pure predicate, `Store` happens inside the goroutine, and `lastFired.Delete(key)` releases the slot on err/panic; direct-invocation path when `e.bus == nil` retains its pre-existing silent behaviour).
...
- **Timestream Query**: SQL parse errors were returned as `fmt.Errorf("SQL parse error: %w", err)`, which the dispatcher converted to a generic 500, preventing AWS SDK clients from receiving `InvalidQueryException` (now the original error is preserved via `logs.Debug` and the client receives an HTTP 400 `InvalidQueryException`); the same treatment applied to `extractTableInfo` schema errors (no table, unsupported table expression/name format), the "only SELECT statements" rejection, and `PrepareQuery` parse errors; store-acquisition failures now return `ErrInternalServer` and cancelled queries return `ErrQueryExecutionError` (were plain `fmt.Errorf`, also collapsing to generic 500); static error in `query_evaluator` switched from `fmt.Errorf` to `errors.New`.
