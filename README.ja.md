...
> **注意：現在ベータ版です。** Vorpalstacksは現在アクティブに開発中です。32のAWSサービスを実装し、2,232件のSDKテスト、47件のサービス間統合テスト、17件のWebSocketテスト（計2,296件、他にPython 631件、TypeScript 2028件、C# 2019件）が通過していますが、すべてのエッジケースとAWSの動作が完全にカバーされているわけではありません。破壊的変更が発生する可能性があります。バグ報告とコントリビューションはもちろん歓迎しています。

AWS互換サービスを提供する軽量エッジ・オンプレミスクラウドプラットフォーム。

## 概要

Vorpalstacksは、完全なAWS接続が利用できない環境でAWS互換サービスを実行可能にすることを目標にしています：
...
![Vorpalstacks 管理コンソール](docs/images/gui.png)

## 特徴

> **本プロジェクトが提供するもの**: モックフレームワークではなく、AWS互換APIの本格的な実装です。各サービスはPebbleDBにデータを保存し、マルチリージョン分離をサポートしています。
...
- **32のAWSサービス**: S3、SQS、SNS、Lambda、DynamoDB、API Gateway、AppSync、Step Functions、WAFv2、Kinesis、KMS、Neptune、Neptune Graphなど
...
- **サービス間連携**: イベント駆動型サービス間通信（47件のサービス間統合テストで検証済み）
...
| Neptune | Broad | プロパティグラフ、openCypher/Gremlin（HTTP + WebSocket）、バルクローダー。SPARQL/RDF未対応 |
| Neptune Graph | Full | 34制御プレーン操作、openCypher、ベクトル検索 |
...
各リージョンにはPebbleDB、Lambdaコード、ログを含む分離されたストレージを配置、グローバルサービスは専用ストレージを共有します。
...
- 一部のエッジケースと公式ドキュメントにないAWS動作は異なる場合があります
...
- **サービス拡張**: AWS IoT Core
- **Terraform**: 28サービスが基本適合性テストに通過 — 詳細と実行方法は[vorpalstacks-conformance-tests](https://github.com/tagumasa/vorpalstacks-conformance-tests)を参照
...
Vorpalstacksは全32サービスをPebbleDBをバックエンドとするネイティブGoバイナリとして実装しており、インタプリタ言語や外部プロセス依存のオーバーヘッドを回避しています。

このアーキテクチャによりコア操作でサブミリ秒のレイテンシを実現し、CI/CDパイプライン内でコンテナ化のオーバーヘッドなしに大量のAPIテスト（Go SDK 2,232件 + 統合 47件 + WS 17件、Python 631件、TypeScript 2028件、C# 2019件）を直接実行可能です。

### ベンチマーク結果（参考値）

プラットフォーム: AMD Ryzen 7 5700U (16コア)、Linux、Go 1.25.8、Pebble v2.1.4

> **注意**: これらの数値は環境に依存します。同一ハードウェア、設定、ワークロードでの比較でなければ意味がないため、参考値です。

| サービス | 操作 | 平均レイテンシ | ops/sec |
|---------|------|---------------|---------|
...
> **注意**: ルートライセンスは本プロジェクトがプロダクションレベルに達した後、MITに変更します。
