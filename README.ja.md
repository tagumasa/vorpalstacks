...
> **注意：ベータ版です。** Vorpalstacksは現在アクティブに開発中な技術デモンストレーションです。29のAWSサービスを実装し、595件のGo SDKテスト（他にPython 631件、TypeScript 629件、C# 606件）が通過していますが、すべてのエッジケースとAWSの動作が完全にカバーされているわけではありません。破壊的変更が発生する可能性があります。バグ報告とコントリビューションはもちろん歓迎しています。
...
- **LocalStackとの比較**: [docs/localstack_vs_vorpalstacks_report.md](docs/localstack_vs_vorpalstacks_report.md)の技術比較を参照
...
- **サービス拡張**: AWS IoT Core, Neptune, AppSync
- **Terraform**: 28サービスで基本的な動作確認済み — 詳細と実行方法は[vorpalstacks-conformance-tests](https://github.com/tagumasa/vorpalstacks-conformance-tests)を参照
...
Vorpalstacksは29のすべてのサービスをPebbleDBをバックエンドとするネイティブGoバイナリとして実装しており、インタプリタ言語や外部プロセス依存のオーバーヘッドを回避しています。

このアーキテクチャによりコア操作がサブミリ秒のレイテンシで実行可能となり、CI/CDパイプライン上でコンテナ化のオーバーヘッドなしに多数のAPIテスト（Go SDK 595件、Python 631件、TypeScript 629件、C# 606件の試験通過済み）を直接実行できます。

### ベンチマーク結果（参考値）

プラットフォーム: AMD Ryzen 7 5700U (16コア), Linux, Go 1.25.8, Pebble v2.1.4

> **注意**: これらの数値は環境に依存します。同一ハードウェア、設定、ワークロードでの比較でなければ他システムとの直接比較は意味をなしません。
...
> **注意**: ルートライセンスは本プロジェクトがプロダクションレベルに達した後、MITに変更します。現在は暫定的にFSL-1.1-MITにしていますが、ある程度の十分な動作確認が取れたタイミングでライセンスを変更する予定です。
