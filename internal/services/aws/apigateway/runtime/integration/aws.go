	...
	if req.IntegrationResponses != nil {
		...
		if respConfig != nil {
			if respConfig.ResponseTemplates != nil {
				contentType := selectResponseContentType(respConfig.ResponseTemplates, resp.Headers, req.Headers)
			...
			if respConfig.ContentHandling != "" {
				resp.Body = applyContentHandlingResponse(resp.Body, respConfig.ContentHandling)
			}
		}
	}
...
func (e *AWSExecutor) buildLambdaProxyEvent(req *IntegrationRequest) *LambdaProxyEvent {
	...
	isBinary := isBinaryContentType(contentType, req.BinaryMediaTypes)
	bodyField := string(req.Body)
	if isBinary && len(req.Body) > 0 {
		bodyField = base64.StdEncoding.EncodeToString(req.Body)
	}
...
// matchIntegrationResponse selects the appropriate integration response
// configuration based on the selectionValue. Per AWS Smithy model:
//   - Lambda non-proxy: selectionValue is the Lambda error result (payload)
//   - HTTP and AWS backends: selectionValue is the HTTP status code string
//   - Mock: selectionValue is the rendered body (no real backend)
//
// If a response has a SelectionPattern, it is matched (as a regex) against
// selectionValue. Responses without a SelectionPattern serve as the default.
// If no match is found, the response keyed by the status code is used.
func matchIntegrationResponse(responses map[string]*IntegrationResponseConfig, selectionValue string, statusCode int) *IntegrationResponseConfig {
