...
func (s *WAFv2Service) ListRegexPatternSets(ctx context.Context, reqCtx *request.RequestContext, req *request.ParsedRequest) (interface{}, error) {
	...
	filtered := result.RegexPatternSets
	if scope != "" {
		tmp := make([]*wafstore.RegexPatternSet, 0, len(filtered))
		for _, rps := range filtered {
			if rps.Scope == scope {
				tmp = append(tmp, rps)
			}
		}
	...
	sets := make([]interface{}, 0, len(filtered))
	for _, rps := range filtered {
