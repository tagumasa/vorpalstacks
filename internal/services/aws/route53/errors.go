...
var storeErrorMappings = []errors.StoreErrorMapping{
	{Store: route53store.ErrHostedZoneExists, AWS: NewHostedZoneAlreadyExistsError("")},
	{Store: route53store.ErrHealthCheckExists, AWS: NewHealthCheckAlreadyExistsError()},
	{Store: route53store.ErrRecordSetNotFound, AWS: errors.NewAWSError("InvalidChangeBatch", "The resource record set does not exist.", http.StatusBadRequest)},
	{Store: route53store.ErrRecordSetExists, AWS: errors.NewAWSError("InvalidChangeBatch", "The resource record set already exists.", http.StatusBadRequest)},
}
...
func mapStoreError(err error) error {
	return errors.MapStoreError(err, storeErrorMappings)
}
