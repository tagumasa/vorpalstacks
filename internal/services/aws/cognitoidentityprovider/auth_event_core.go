...
// AdminListUserAuthEvents. Params holds the raw request parameter map; the
// MaxResults limit is read from it inside the Core.
...
// AdminUpdateAuthEventFeedbackInput carries the wire parameters of
...
// UpdateAuthEventFeedbackInput carries the wire parameters of
...
// adminListUserAuthEventsCore lists a user's recorded authentication
// events with offset pagination.
func (s *CognitoService) adminListUserAuthEventsCore(reqCtx *request.RequestContext, in AdminListUserAuthEventsInput) (interface{}, error) {
	...
	result, err := store.ListAuthEventsPaginated(in.UserPoolID, user.ID, storecommon.ListOptions{
		MaxItems: maxResults,
		Marker:   in.NextToken,
	})
	if err != nil {
...
// adminUpdateAuthEventFeedbackCore records admin feedback on an auth event.
func (s *CognitoService) adminUpdateAuthEventFeedbackCore(reqCtx *request.RequestContext, in AdminUpdateAuthEventFeedbackInput) (interface{}, error) {
	if in.UserPoolID == "" || in.Username == "" || in.EventID == "" || in.FeedbackValue == "" {
		return nil, ErrInvalidParameter
	}
	if in.FeedbackValue != "Valid" && in.FeedbackValue != "Invalid" {
		return nil, ErrInvalidParameter
	}
	...
	event, err := store.GetAuthEvent(in.UserPoolID, user.ID, in.EventID)
	if err != nil {
		return nil, ErrResourceNotFound
	}

	event.FeedbackValue = in.FeedbackValue
...
// updateAuthEventFeedbackCore records user feedback on an auth event,
// resolving the caller from an access token.
func (s *CognitoService) updateAuthEventFeedbackCore(reqCtx *request.RequestContext, in UpdateAuthEventFeedbackInput) (interface{}, error) {
	if in.AccessToken == "" || in.EventID == "" || in.FeedbackValue == "" {
		return nil, ErrInvalidParameter
	}
	if in.FeedbackValue != "Valid" && in.FeedbackValue != "Invalid" {
		return nil, ErrInvalidParameter
	...
	event, err := store.GetAuthEvent(user.UserPoolID, user.ID, in.EventID)
	if err != nil {
		return nil, ErrResourceNotFound
	}
