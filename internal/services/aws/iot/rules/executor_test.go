...
func TestWorkerPoolLifecycle(t *testing.T) {
	...
	dispatcher := func(_ context.Context, ruleName, topic string, _ []map[string]interface{}, _ map[string]interface{}, payload map[string]interface{}, _ int) error {
		dispatched.Add(1)
		return nil
	}
...
// TestWorkerPoolDrainUsesFreshContext verifies that drain-path tasks receive
// a non-cancelled context. A cooperative dispatcher that rejects a cancelled
// context should still process all queued tasks during shutdown.
func TestWorkerPoolDrainUsesFreshContext(t *testing.T) {
	var mu sync.Mutex
	dispatched := []string{}

	dispatcher := func(ctx context.Context, ruleName string, _ string, _ []map[string]interface{}, _ map[string]interface{}, _ map[string]interface{}, _ int) error {
		if ctx.Err() != nil {
			return ctx.Err()
		}
		mu.Lock()
	...
	mu.Lock()
	if len(dispatched) != 5 {
		t.Errorf("drain dispatched count = %d, want 5 (ctx should not be cancelled during drain)", len(dispatched))
	}
	mu.Unlock()
}
...
func TestExecutorRulesCount(t *testing.T) {
	...
	e.AddRule("r1", "test/#", "SELECT * FROM 'test/#'", nil, nil)
	e.AddRule("r2", "foo/#", "SELECT * FROM 'foo/#'", nil, nil)
...
func TestExecutorEndToEnd_SQLParseTopicMatchDispatch(t *testing.T) {
	...
	err := e.AddRule("temp_high", "sensors/+/temperature", "SELECT temperature, location FROM 'sensors/+/temperature' WHERE temperature > 30", nil, nil)
	if err != nil {
...
func TestExecutorEndToEnd_DisableDeleteRemovesRule(t *testing.T) {
	...
	e.AddRule("alert", "alerts/#", "SELECT * FROM 'alerts/#'", nil, nil)
...
func TestExecutorEndToEnd_ReplaceRule(t *testing.T) {
	...
	e.AddRule("r1", "old/#", "SELECT * FROM 'old/#'", nil, nil)
	...
	e.RemoveRule("r1")
	e.AddRule("r1", "new/#", "SELECT * FROM 'new/#'", nil, nil)
