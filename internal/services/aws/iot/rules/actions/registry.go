...
var actionRegistry = map[string]ActionHandler{
	...
	"cloudwatch":     (*Dispatcher).dispatchCloudWatch,
	"cloudwatchLogs": (*Dispatcher).dispatchCloudWatchLogs,
	"eventbridge":    (*Dispatcher).dispatchEventBridge,
	...
	"timestream": func(_ *Dispatcher, _ context.Context, _ *ActionConfig, _ *ActionPayload) error {
		return fmt.Errorf("timestream action not supported in vorpalstacks")
	},
}
