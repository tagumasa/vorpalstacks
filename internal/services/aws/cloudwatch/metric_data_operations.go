...
// isBasicStatistic returns true for the five standard CloudWatch statistics.
func isBasicStatistic(lowerStat string) bool {
	switch lowerStat {
...
// extractStatValue pulls the requested statistic value from a
// MetricStatistics struct, handling both basic stats (Sum, Average, etc.)
// and extended stats (percentiles, IQM, etc.).
func extractStatValue(s *cwstore.MetricStatistics, statLower, statOriginal string, isExtended bool) float64 {
	if isExtended {
		if s.ExtendedStats != nil {
			if v, ok := s.ExtendedStats[statOriginal]; ok {
				return v
			}
		}
		return 0
...
// expandToPeriodGrid maps real data points onto a period-aligned time
// grid from startTime to endTime. Periods with no data are filled with
// NaN so that FILL() and other metric math functions can detect and
// replace them. This mirrors CloudWatch's internal behaviour where
// GetMetricData operates on a uniform period grid rather than sparse
// data points.
func expandToPeriodGrid(points []metricmath.DataPoint, startTime, endTime time.Time, period int32) []metricmath.DataPoint {
	periodDuration := time.Duration(period) * time.Second
	gridStart := startTime.Truncate(periodDuration)
	if gridStart.Before(startTime) {
		gridStart = gridStart.Add(periodDuration)
	}

	pointMap := make(map[int64]float64, len(points))
	for _, p := range points {
		pointMap[p.Timestamp.Unix()] = p.Value
	}

	var result []metricmath.DataPoint
	for t := gridStart; !t.After(endTime); t = t.Add(periodDuration) {
		if val, ok := pointMap[t.Unix()]; ok {
			result = append(result, metricmath.DataPoint{Timestamp: t, Value: val})
		} else {
			result = append(result, metricmath.DataPoint{Timestamp: t, Value: math.NaN()})
		}
	}
	return result
}

// filterNaN removes data points with NaN values from a series. AWS
// GetMetricData omits periods with no data from the Values/Timestamps
// arrays in the response.
func filterNaN(points []metricmath.DataPoint) []metricmath.DataPoint {
	filtered := make([]metricmath.DataPoint, 0, len(points))
	for _, p := range points {
		...
		for _, dp := range stats {
			...
			default:
				if dp.ExtendedStats != nil {
					if ev, ok := dp.ExtendedStats[m.Stat]; ok {
						val = ev
					}
				}
			}
