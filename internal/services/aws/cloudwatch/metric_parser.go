	...
	if queriesList, ok := queriesRaw.([]interface{}); ok {
		for _, q := range queriesList {
			if qMap, ok := q.(map[string]interface{}); ok {
				...
				query.Expression = request.GetStringParam(qMap, "Expression")
				if query.Expression == "" {
					query.Expression = request.GetStringParam(qMap, "expression")
				}
				query.Label = request.GetStringParam(qMap, "Label")
				if query.Label == "" {
					query.Label = request.GetStringParam(qMap, "label")
				}
				if retData, ok := qMap["ReturnData"]; ok {
					if b, ok := retData.(bool); ok {
						query.ReturnData = b
					}
				} else if retData, ok := qMap["returnData"]; ok {
					if b, ok := retData.(bool); ok {
						query.ReturnData = b
					}
				} else {
				...
				query.Period = int32(request.GetIntParam(qMap, "Period"))
				if query.Period == 0 {
					query.Period = int32(request.GetIntParam(qMap, "period"))
				}
...
func metricDataQueriesToResponse(queries []cwstore.MetricDataQuery) []map[string]interface{} {
	...
	for _, q := range queries {
		...
		if q.MetricStat != nil {
			...
			if q.MetricStat.Unit != "" {
				stat["Unit"] = string(q.MetricStat.Unit)
			}
			...
			if len(q.MetricStat.Metric.Dimensions) > 0 {
				dims := make([]map[string]string, len(q.MetricStat.Metric.Dimensions))
				for i, d := range q.MetricStat.Metric.Dimensions {
