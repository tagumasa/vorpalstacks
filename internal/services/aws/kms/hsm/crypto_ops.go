...
func (c *cryptoOps) sign(keyID string, message []byte, algorithm SigningAlgorithm, messageType MessageType) (*SignResult, error) {
	key, exists := c.keys[keyID]
	if !exists {
		return nil, ErrKeyNotFound
	}
	...
	if key.rsaKey != nil {
		hash, err := digestForAlgorithm(algorithm, message, messageType)
		if err != nil {
			return nil, err
		}
		switch algorithm {
...
func (c *cryptoOps) verify(keyID string, message, signature []byte, algorithm SigningAlgorithm, messageType MessageType) (bool, error) {
	key, exists := c.keys[keyID]
	if !exists {
...
func expectedDigestLength(algorithm SigningAlgorithm) int {
	switch algorithm {
...
func digestForAlgorithm(algorithm SigningAlgorithm, message []byte, messageType MessageType) ([]byte, error) {
	if messageType == MessageTypeDigest {
		expected := expectedDigestLength(algorithm)
		if len(message) != expected {
			return nil, ErrInvalidDigestLength
		}
		return message, nil
	}
	return hashForAlgorithm(algorithm, message), nil
}
