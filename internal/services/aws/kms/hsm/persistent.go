...
func (b *PersistentBackend) Sign(keyID string, message []byte, algorithm SigningAlgorithm, messageType MessageType) (*SignResult, error) {
	b.mu.RLock()
	defer b.mu.RUnlock()
...
func (b *PersistentBackend) Verify(keyID string, message, signature []byte, algorithm SigningAlgorithm, messageType MessageType) (bool, error) {
	b.mu.RLock()
	defer b.mu.RUnlock()
