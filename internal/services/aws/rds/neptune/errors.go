...
func translateStoreError(err error) error {
	...
	case errors.Is(err, neptunestore.ErrDBSnapshotNotFound):
		return awserrors.NewAWSError("DBSnapshotNotFoundFault", "DBSnapshot not found: "+extractStoreMsg(err), http.StatusNotFound)
	...
	case errors.Is(err, neptunestore.ErrDBSnapshotAlreadyExists):
		return awserrors.NewAWSError("DBSnapshotAlreadyExistsFault", "DBSnapshot already exists", http.StatusConflict)
