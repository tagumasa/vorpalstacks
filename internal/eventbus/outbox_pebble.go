...
// recover entries left behind by a crash or panic during processing.
func (s *PebbleOutboxStore) ResetStaleProcessing(ctx context.Context) (int, error) {
	...
	for iter.First(); iter.Valid(); iter.Next() {
		var entry OutboxEntry
		if err := json.Unmarshal(iter.Value(), &entry); err != nil {
			s.logWarn("skipping malformed outbox entry during stale reset", "key", string(iter.Key()), "error", err)
			continue
		}

		if entry.Status != OutboxProcessing {
			continue
		}
		...
		newVal, err := json.Marshal(&entry)
		if err != nil {
			s.logWarn("failed to marshal stale entry", "event_id", entry.EventID, "error", err)
			continue
		}

		if err := s.db.Set(outboxKey(entry.EventID), newVal, pebble.Sync); err != nil {
			s.logWarn("failed to persist stale entry reset", "event_id", entry.EventID, "error", err)
			continue
		}
		count++
	}
	...
	if err := iter.Error(); err != nil {
		return count, fmt.Errorf("pebble outbox: iterate stale reset: %w", err)
	}
...
// Cleanup purges delivered, failed, and stale pending entries older than the
// given retention thresholds, returning the number of entries removed.
func (s *PebbleOutboxStore) Cleanup(ctx context.Context, deliveredBefore time.Time, failedBefore time.Time, pendingBefore time.Time) (int, error) {
	...
	for iter.First(); iter.Valid(); iter.Next() {
		...
		case OutboxPending:
			if entry.CreatedAt.Before(pendingBefore) {
				toDelete = append(toDelete, deleteEntry{eventID: eventID, idxKey: key})
			}
		}
	}
