...
// PendingRetention is how long pending outbox entries are kept before purging.
// Pending entries accumulate when the server is killed (e.g. pkill -9)
// while events are still in the async channel. Without this, they persist
// forever and clog recovery on the next start.
// Overridden to 1m in TEST_MODE.
var PendingRetention = 30 * time.Minute

func init() {
	if os.Getenv("TEST_MODE") == "true" {
		CleanupInterval = 30 * time.Second
		PendingRetention = 1 * time.Minute
	}
}
...
func (b *EventBus) DynamoDBStreamsInvoker() DynamoDBStreamsInvoker { return b.dynamoDBStreamsInvoker }
...
func (b *EventBus) directWorker() {
	...
	for {
		...
		case d := <-b.directCh:
			...
			func() {
				defer b.releaseSemaphores(d.sub)
...
func (b *EventBus) cleanupLoop() {
	...
	for {
		...
		case <-ticker.C:
			...
			pendingBefore := now.Add(-PendingRetention)
			n, err := b.outbox.Cleanup(context.Background(), deliveredBefore, failedBefore, pendingBefore)
			if err != nil {
