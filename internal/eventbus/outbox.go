...
type OutboxStore interface {
	...
	// (left behind by a crash or panic) and resets them to
	// OutboxPending so they can be recovered on the next Start.
	// Returns the number of entries reset.
	ResetStaleProcessing(ctx context.Context) (int, error)
	Delete(ctx context.Context, eventID string) error
	Cleanup(ctx context.Context, deliveredBefore time.Time, failedBefore time.Time, pendingBefore time.Time) (int, error)
	Close() error
}
