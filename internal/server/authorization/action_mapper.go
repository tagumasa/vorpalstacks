...
func (m *ActionMapper) registerEventBridgeMappings() {
	// EventBridge handlers register under "eventbridge", but IAM actions
	// use the "events:" prefix (legacy CloudWatch Events name).
	m.RegisterWildcard("eventbridge", "events")
}
...
func (m *ActionMapper) registerCloudWatchMappings() {
	// CloudWatch Logs handlers register under "logs".
	m.RegisterWildcard("logs", "logs")
	// CloudWatch handlers register under "monitoring" (the signing service
	// name), but IAM actions use the "cloudwatch:" prefix.
	m.RegisterWildcard("monitoring", "cloudwatch")
}
...
func (m *ActionMapper) registerSESMappings() {
...
func (m *ActionMapper) registerRDSDataMappings() {
	// RDS Data API handlers register under "rdsdata", but IAM actions
	// use the "rds-data:" prefix.
	m.RegisterWildcard("rdsdata", "rds-data")
}
