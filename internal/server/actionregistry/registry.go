...
// IAMActions returns a copy of the default IAM action names. Guard tests
// compare it with the handler registrations of the IAM service in both
// directions.
func IAMActions() []string {
	actions := make([]string, len(defaultIAMActions))
	copy(actions, defaultIAMActions)
	return actions
}

func (r *ActionRegistry) initDefaults() {
	r.Register("iam", defaultIAMActions)
