...
// The implementations live in internal/utils/vecdist, the shared owner of the
// pure distance algorithms; this alias keeps the graphengine API stable.
type DistanceMetric = vecdist.DistanceMetric

// Distance metric constants.
const (
	L2Squared        = vecdist.L2Squared
	L2               = vecdist.L2
	CosineSimilarity = vecdist.CosineSimilarity
	CosineDistance   = vecdist.CosineDistance
	DotProduct       = vecdist.DotProduct
)
...
func ParseDistanceMetric(s string) DistanceMetric {
	return vecdist.ParseDistanceMetric(s)
}
...
func ComputeDistance(a, b []float64, metric DistanceMetric) (float64, error) {
	return vecdist.ComputeDistance(a, b, metric)
}
