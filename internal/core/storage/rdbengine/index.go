...
// addIndexEntries inserts index entries for a row. Called during InsertRow.
func (s *Store) addIndexEntries(db, table string, pk []byte, row Row) error {
...
// removeIndexEntries removes index entries for a row. Called during DeleteRow.
func (s *Store) removeIndexEntries(db, table string, pk []byte, row Row) error {
	indexes, err := s.listIndexDefs(db, table)
	if err != nil {
		return err
	}
...
// updateIndexEntries removes old and adds new index entries. Called during UpdateRow.
func (s *Store) updateIndexEntries(db, table string, pk []byte, oldRow, newRow Row) error {
	if err := s.removeIndexEntries(db, table, pk, oldRow); err != nil {
		return err
	}
	return s.addIndexEntries(db, table, pk, newRow)
}
