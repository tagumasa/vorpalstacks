...
func AlarmModelToProto(a *AlarmModel) (*pb.AlarmModel, error) {
	def, err := mapToStruct(a.AlarmModelDefinition)
	if err != nil {
		return nil, fmt.Errorf("alarm model definition: %w", err)
	}
	return &pb.AlarmModel{
		AlarmModelName:        a.AlarmModelName,
		AlarmModelArn:         a.AlarmModelARN,
		AlarmModelDescription: a.AlarmModelDescription,
		RoleArn:               a.RoleARN,
		AlarmModelDefinition:  def,
		Tags:                  a.Tags,
		CreationDate:          timeToProto(a.CreationDate),
		LastModifiedDate:      timeToProto(a.LastModifiedDate),
		Status:                a.Status,
		Severity:              a.Severity,
		AlarmModelVersion:     a.AlarmModelVersion,
	}, nil
}

func ProtoToAlarmModel(p *pb.AlarmModel) *AlarmModel {
	return &AlarmModel{
		AlarmModelName:        p.AlarmModelName,
		AlarmModelARN:         p.AlarmModelArn,
		AlarmModelDescription: p.AlarmModelDescription,
		RoleARN:               p.RoleArn,
		AlarmModelDefinition:  structToMap(p.AlarmModelDefinition),
		Tags:                  p.Tags,
		CreationDate:          protoToTime(p.CreationDate),
		LastModifiedDate:      protoToTime(p.LastModifiedDate),
		Status:                p.Status,
...
// AlarmModel represents an IoT Events alarm model.
type AlarmModel struct {
	AlarmModelName        string
	AlarmModelARN         string
	AlarmModelDescription string
	RoleARN               string
	AlarmModelDefinition  map[string]interface{}
	Tags                  map[string]string
	CreationDate          time.Time
	LastModifiedDate      time.Time
	Status                string
