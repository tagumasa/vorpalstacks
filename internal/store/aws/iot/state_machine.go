...
// SetActionCallback installs or replaces the detector action dispatch
// callback. The write is guarded by sm.mu so that concurrent EvaluateEvent
// readers observe a consistent callback. Intended for one-shot
// initialisation after GetOrCreateStore constructs the singleton store.
func (sm *DetectorStateMachine) SetActionCallback(fn func(modelName, key, actionType string, payload map[string]interface{})) {
	sm.mu.Lock()
	defer sm.mu.Unlock()
	sm.onAction = fn
}
...
func (sm *DetectorStateMachine) EvaluateEvent(ctx context.Context, modelName, key string, payload map[string]interface{}) error {
	...
	// Snapshot onAction under the read lock so that a concurrent
	// SetActionCallback write cannot race with this read. Dispatch still
	// runs outside the lock (see Phase 2) to avoid head-of-line blocking
	// on slow eventbus calls.
	sm.mu.RLock()
	onAction := sm.onAction
	sm.mu.RUnlock()
	...
	for _, p := range pending {
		if onAction != nil {
			onAction(modelName, key, p.actionType, p.params)
		}
	}
	return nil
}
...
// If modelName is empty, instances across all models are returned.
// Each entry contains the model name, key, state name, and last update time.
func (sm *DetectorStateMachine) ListDetectorInstances(modelName string) []map[string]interface{} {
	sm.mu.RLock()
	defer sm.mu.RUnlock()
	...
	if modelName != "" {
		...
		for key, ds := range instanceMap {
			result = append(result, map[string]interface{}{
				"detectorModelName": modelName,
				"keyValue":          key,
				"stateName":         ds.stateName,
				"lastUpdateTime":    ds.lastUpdated.Unix(),
			})
		}
		return result
	}
	for mName, instanceMap := range sm.detectors {
		for key, ds := range instanceMap {
			result = append(result, map[string]interface{}{
				"detectorModelName": mName,
				"keyValue":          key,
				"stateName":         ds.stateName,
				"lastUpdateTime":    ds.lastUpdated.Unix(),
			})
