...
func (s *ObjectStore) GetWithVersion(ctx context.Context, bucket, key, versionId string) (io.ReadCloser, *Object, error) {
	...
	if effectiveVersionId != "" {
		...
		if err = s.BaseStore.GetProto(latestKey, &pbObj); err != nil {
			...
			// Pre-versioning objects are stored with versionId "null" and have no _latest pointer.
	...
	if isVersioned {
		...
		if blobVersionId == "" || blobVersionId == "null" {
			blobVersionId = "null"
		}
...
func (s *ObjectStore) SetACLWithVersion(bucket, key, versionId string, acp *AccessControlPolicy) error {
	...
	var storageKey string
	if effectiveVersionId != "" {
		sk := s.versionedStorageKey(bucket, key, effectiveVersionId)
		if err := s.BaseStore.GetProto(sk, &pbObj); err != nil {
