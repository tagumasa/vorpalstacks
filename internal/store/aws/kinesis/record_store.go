...
func (s *KinesisStore) PutRecordWithShardSelection(streamName, partitionKey, data, explicitHashKey string) (*Record, string, error) {
	...
	var targetShard *Shard
	if explicitHashKey != "" {
		ts, err := s.selectShardByHashKey(activeShards, explicitHashKey)
		if err != nil {
...
func (s *KinesisStore) PutRecords(streamName string, records []PutRecordRequest) ([]PutRecordResult, error) {
	...
	for i, req := range records {
		var shard *Shard
		if req.ExplicitHashKey != "" {
			hs, herr := s.selectShardByHashKey(activeShards, req.ExplicitHashKey)
			if herr != nil {
				results[i] = PutRecordResult{ErrorCode: "InvalidArgumentException"}
				continue
			}
			shard = hs
		}
		if shard == nil {
			shard, _ = s.selectShardByPartitionKey(activeShards, req.PartitionKey)
		}
		if shard == nil {
...
func (s *KinesisStore) selectShardByHashKey(shards []*Shard, hashKey string) (*Shard, error) {
	...
	return nil, fmt.Errorf("no shard found for explicit hash key %s", hashKey)
}
