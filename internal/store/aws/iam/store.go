...
// group memberships, MFA devices, signing certificates, SSH public keys,
// service-specific credentials) to the new name.  All migrations run
// before the user record is swapped; if any step fails, completed steps
// are reverted so that resources remain under the old name.
func (s *IAMStore) RenameUser(oldName, newName, newPath string) error {
	...
	return s.users.kl.WithLock(oldName, func() error {
		...
		if needsRename {
			// Phase 1: Migrate all associated resources from oldName to
			// newName BEFORE swapping the user record.  If any migration
			// step fails, completed steps are reverted in reverse order so
			// that all resources end up back under oldName.
			migrateAccessKeys := func(from, to string) error {
				keys, err := s.accessKeys.ListByUserNameWithSecret(from)
				if err != nil {
					return err
				}
			...
			migrateLoginProfile := func(from, to string) error {
				if !s.loginProfiles.Exists(from) {
					return nil
				}
				profile, err := s.loginProfiles.Get(from)
				if err != nil {
					return err
				}
				profile.UserName = to
				...
				return s.loginProfiles.Delete(from)
			}
			...
			steps := []migrationStep{
				{func() error { return migrateAccessKeys(oldName, newName) }, func() error { return migrateAccessKeys(newName, oldName) }},
				{func() error { return migrateLoginProfile(oldName, newName) }, func() error { return migrateLoginProfile(newName, oldName) }},
				{func() error { return s.inlinePolicies.MigratePrincipal(oldName, newName, "user") }, func() error { return s.inlinePolicies.MigratePrincipal(newName, oldName, "user") }},
				{func() error { return s.attachedPolicies.MigratePrincipal(oldName, newName, "user") }, func() error { return s.attachedPolicies.MigratePrincipal(newName, oldName, "user") }},
				{func() error { return s.userGroups.MigrateUser(oldName, newName) }, func() error { return s.userGroups.MigrateUser(newName, oldName) }},
				{func() error { return s.mfaDevices.MigrateUser(oldName, newName) }, func() error { return s.mfaDevices.MigrateUser(newName, oldName) }},
				{func() error { return s.signingCertificates.MigrateUser(oldName, newName) }, func() error { return s.signingCertificates.MigrateUser(newName, oldName) }},
				{func() error { return s.sshPublicKeys.MigrateUser(oldName, newName) }, func() error { return s.sshPublicKeys.MigrateUser(newName, oldName) }},
				{func() error { return s.serviceSpecificCreds.MigrateUser(oldName, newName) }, func() error { return s.serviceSpecificCreds.MigrateUser(newName, oldName) }},
			}
