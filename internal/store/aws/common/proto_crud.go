...
type ProtoStoreConfig[D any] struct {
	...
	ToDomain     func(proto.Message) (*D, error)
	ToProto      func(*D) (proto.Message, error)
...
func (s *ProtoStore[D]) Create(entity *D) error {
	...
	pm, err := s.toProto(entity)
	if err != nil {
		return err
	}
	return s.store.PutProto(id, pm)
}
...
func (s *ProtoStore[D]) UpdateDirect(entity *D) error {
	...
	return s.store.PutProto(s.idFunc(entity), pm)
}
...
func (s *ProtoStore[D]) List() ([]*D, error) {
	...
	err := s.store.ForEach(func(key string, value []byte) error {
		...
		d, err := s.toDomain(pb)
		if err != nil {
			logs.Warn("domain conversion failed, skipping entry", logs.String("key", key), logs.Err(err))
			return nil
		}
