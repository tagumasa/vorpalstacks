...
func TestEvaluateTrustPolicy(t *testing.T) {
	...
	// Deny overrides Allow when both match the same principal/action.
	docDenyAndAllow := &policy.Document{
		Statement: []policy.Statement{
			{
				Effect: policy.EffectAllow,
				Principal: &policy.Principal{
	...
	err = EvaluateTrustPolicy(docDenyAndAllow, string(ServicePrincipalLambda), evalCtx)
	if err != ErrRoleCannotBeAssumed {
		t.Errorf("EvaluateTrustPolicy() Deny+Allow expected ErrRoleCannotBeAssumed, got %v", err)
	}
}
