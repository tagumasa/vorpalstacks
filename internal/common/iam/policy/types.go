...
// matchActionPattern checks if an action matches an IAM action pattern that
// may contain glob-style '*' wildcards anywhere in the string (e.g.
// "s3:Get*", "s3:Get*Object").
func matchActionPattern(pattern, action string) bool {
	return wildcardMatch(pattern, action)
}

// matchArnPattern checks if an ARN matches an IAM resource pattern that
// may contain glob-style '*' wildcards anywhere in the string.
