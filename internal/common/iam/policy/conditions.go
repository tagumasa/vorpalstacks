...
func matchArnLike(pattern, arn string) bool {
	...
	for i, seg := range segments {
		...
		if strings.Contains(seg, "*") {
			if !wildcardMatch(seg, arnSegments[i]) {
				return false
			}
...
// parseFloat parses a numeric string into a float64, handling integers,
// decimals, negative values, and exponential notation (e.g. "1e5", "1.5e-3").
