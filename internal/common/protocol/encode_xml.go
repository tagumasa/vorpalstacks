...
// lowerFirst lowercases the first rune of a string. EC2 Query protocol uses
// lowerCamelCase element names (e.g. "groupId"), unlike standard AWS Query
// which uses PascalCase (e.g. "GroupId").
...
func encodeMapToXMLEC2(builder *strings.Builder, data map[string]interface{}) {
	for key, value := range data {
		...
		case int, int8, int16, int32, int64, uint, uint8, uint16, uint32, uint64, float32, float64:
			builder.WriteString(numberToString(v))
		case bool:
			if v {
...
// encodeValueToXMLEC2 is the EC2-specific variant of encodeValueToXML. It
// delegates to encodeMapToXMLEC2 for map types (applying lowerCamelCase keys)
// and to encodeValueToXML for all other types.
func encodeValueToXMLEC2(builder *strings.Builder, v interface{}) {
	switch val := v.(type) {
	case map[string]interface{}:
		encodeMapToXMLEC2(builder, val)
	case map[string]string:
		for k, s := range val {
			builder.WriteString("<entry>")
			builder.WriteString("<key>" + escapeXML(k) + "</key>")
	...
	case time.Time:
		builder.WriteString(escapeXML(val.UTC().Format(time.RFC3339)))
	default:
		// Handle structs and reflect maps with EC2 lowerCamelCase keys.
		rv := reflect.ValueOf(v)
		for rv.Kind() == reflect.Ptr {
			if rv.IsNil() {
				return
		...
		switch rv.Kind() {
		case reflect.Struct:
			encodeStructToXMLEC2(builder, rv)
		case reflect.Map:
...
// encodeStructToXMLEC2 is the EC2 variant of encodeStructToXML, using
// lowerFirst for element names and encodeValueToXMLEC2 for values.
func encodeStructToXMLEC2(builder *strings.Builder, rv reflect.Value) {
	rt := rv.Type()
	for i := 0; i < rt.NumField(); i++ {
		field := rt.Field(i)
		if !field.IsExported() {
		...
		xmlKey := lowerFirst(key)
		builder.WriteString("<" + xmlKey + ">")
		encodeValueToXMLEC2(builder, fv.Interface())
		builder.WriteString("</" + xmlKey + ">")
	}
}

// encodeReflectMapToXMLEC2 is the EC2 variant of encodeReflectMapToXML.
func encodeReflectMapToXMLEC2(builder *strings.Builder, rv reflect.Value) {
	for _, k := range rv.MapKeys() {
		var rawKey string
		if k.Kind() == reflect.String {
			rawKey = k.String()
		...
		xmlKey := lowerFirst(rawKey)
		builder.WriteString("<" + xmlKey + ">")
		encodeValueToXMLEC2(builder, rv.MapIndex(k).Interface())
		builder.WriteString("</" + xmlKey + ">")
	}
}
...
// response root element. EC2 element names are lowerCamelCase (e.g.
// "groupId", not "GroupId"), so a dedicated encoder is used.
func EncodeEC2QueryXMLResponse(w http.ResponseWriter, operationName string, response interface{}) error {
	...
	if response != nil {
		encodeValueToXMLEC2(&xmlBuilder, response)
	}
