  ...
  switch (type) {
    ...
    case "M": {
      ...
      try {
        ...
        throw new Error("Map value must be valid JSON");
      }
      if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) throw new Error("Map value must be a JSON object");
      const m: Record<string, AttributeValue> = {};
...
export function rowToPlain(row: AttrRow): unknown {
  ...
  // Per-type conversion avoids JSON.parse corrupting S values like
  // "true"/"42"/"null" into bool/number/null types.
  switch (row.type) {
    case "S":
      return row.value;
    case "N":
    ...
    case "NS":
      return row.value.split(",").map((s) => Number(s.trim())).filter((n) => !isNaN(n));
    default:
      return row.value;
  }
}
