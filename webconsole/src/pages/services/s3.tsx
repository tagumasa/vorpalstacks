...
/**
 * Image preview sub-component. Manages Blob object URL lifecycle via
 * useMemo + useEffect cleanup to prevent memory leaks on re-render or
 * tab switch. The parent renderTabContent cannot use hooks because it
 * is called conditionally.
 */
function ImagePreview({ bodyBytes, contentType, displayName }: {
...
export function S3Page() {
  ...
  const objectColumns: ColumnDef<ObjectRow, any>[] = [
    ...
    { accessorKey: "size", header: t("services.s3.sizeHeader"), cell: ({ getValue }) => <span className="cell-mono">{formatBytes(Number(getValue()))}</span>, size: 100 },
    ...
    const renderTabContent = () => {
      switch (detailTab) {
        ...
        case "raw": {
          ...
          if (!previewData) return <div className="detail-panel-empty">{previewFetching ? t("common.loading") : t("services.s3.noPreviewData")}</div>;
          const bodyBytes = previewData.body as Uint8Array;
          if (isImageFile(selectedFile.key)) {
            return (
              <ImagePreview
                bodyBytes={bodyBytes}
                contentType={inferContentType(selectedFile.key)}
                displayName={selectedFile.displayKey}
              />
            );
          }
      ...
      <ServiceDeleteDialog
        ...
        onConfirm={async () => {
          const results = await Promise.allSettled(
            Array.from(selectedBucketNames).map((name) =>
              deleteBucketMutation.mutateAsync(name),
            ),
          );
