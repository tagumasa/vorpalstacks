...
export function CloudWatchPage() {
  ...
  const { tags: itemTags, isLoading: tagsLoading, addTags, removeTag, isPending: tagsPending } = useTags(
    ...
    selectedItem?.alarmarn || undefined,
  );
      ...
      <DetailPanel title={selectedItem.alarmname} titleIcon="📊" tabs={[{ key: "detail", label: t("common.tabDetail") }, { key: "json", label: t("common.rawJson") }]} activeTab={detailTab} onTabChange={(k) => setDetailTab(k as DetailTab)} actions={<button className="btn btn-danger btn-sm" onClick={() => setShowDelete(true)}>{t("common.delete")}</button>}>
        {detailTab === "detail" ? (
          <><table className="settings-table"><tbody>
            <tr><td className="detail-label-fixed">{t("common.fields.alarm")}</td><td className="cell-mono">{selectedItem.alarmname}</td></tr>
            <tr><td className="detail-label">{t("common.fields.state")}</td><td><span className="badge">{stateKey ? t(stateKey) ?? String(selectedItem.statevalue) : String(selectedItem.statevalue)}</span></td></tr>
            <tr><td className="detail-label">{t("common.fields.metric")}</td><td>{selectedItem.metricname || "\u2014"}</td></tr>
            <tr><td className="detail-label">{t("common.fields.namespace")}</td><td>{selectedItem.namespace || "\u2014"}</td></tr>
            <tr><td className="detail-label">{t("common.fields.statistic")}</td><td>{statKey ? t(statKey) ?? String(selectedItem.statistic) : String(selectedItem.statistic)}</td></tr>
            <tr><td className="detail-label">{t("common.fields.period")}</td><td>{selectedItem.period}s</td></tr>
            <tr><td className="detail-label">{t("common.fields.evalPeriods")}</td><td>{selectedItem.evaluationperiods}</td></tr>
            <tr><td className="detail-label">{t("common.fields.threshold")}</td><td>{selectedItem.threshold}</td></tr>
            <tr><td className="detail-label">{t("common.fields.comparison")}</td><td>{compKey ? t(compKey) ?? String(selectedItem.comparisonoperator) : String(selectedItem.comparisonoperator)}</td></tr>
            {selectedItem.alarmdescription && <tr><td className="detail-label">{t("common.fields.description")}</td><td>{selectedItem.alarmdescription}</td></tr>}
          </tbody></table>
