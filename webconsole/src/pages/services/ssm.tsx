  ...
  const { tags: itemTags, isLoading: tagsLoading, addTags, removeTag, isPending: tagsPending } = useTags(
    {
      ...
      fetchTags: async (paramName: string) => {
        const res = await client.listTagsForResource({ resourceid: paramName, resourcetype: ResourceTypeForTagging.PARAMETER });
        return (res.taglist ?? []).map((t) => ({ key: t.key, value: t.value }));
      },
      tagResource: async (paramName: string, tags) => {
        await client.addTagsToResource({ resourceid: paramName, resourcetype: ResourceTypeForTagging.PARAMETER, tags: tags.map((t) => ({ key: t.key, value: t.value })) });
      },
      untagResource: async (paramName: string, tagKeys: string[]) => {
        await client.removeTagsFromResource({ resourceid: paramName, resourcetype: ResourceTypeForTagging.PARAMETER, tagkeys: tagKeys });
      },
    },
        ...
        {detailTab === "detail" ? (
          <><table className="settings-table">
            <tbody>
              ...
              <tr><td className="detail-label">{t("common.fields.type")}</td><td>{(() => {
                const labels: Record<number, string> = {
                  [ParameterType.SECURE_STRING]: t("services.ssm.paramTypeSecureString"),
                  [ParameterType.STRING_LIST]: t("services.ssm.paramTypeStringList"),
                  [ParameterType.STRING]: t("services.ssm.paramTypeString"),
              ...
              <tr><td className="detail-label">{t("common.fields.version")}</td><td>{selectedItem.version}</td></tr>
              <tr><td className="detail-label">{t("common.fields.tier")}</td><td>{(() => {
                const labels: Record<number, string> = {
                  [ParameterTier.STANDARD]: t("services.ssm.tierStandard"),
                  [ParameterTier.ADVANCED]: t("services.ssm.tierAdvanced"),
                  [ParameterTier.INTELLIGENT_TIERING]: t("services.ssm.tierIntelligent"),
              ...
              <tr><td className="detail-label">{t("common.fields.dataType")}</td><td>{selectedItem.datatype || "text"}</td></tr>
              {selectedItem.lastmodifieddate && (
                <tr><td className="detail-label">{t("common.fields.lastModified")}</td><td>{fmtDate(selectedItem.lastmodifieddate, i18n.language)}</td></tr>
              )}
            </tbody>
          </table>
