      ...
      <DetailPanel title={selectedItem.name} titleIcon="🌐" tabs={[{ key: "detail", label: "Detail" }, { key: "json", label: t("common.rawJson") ?? "JSON" }]} activeTab={detailTab} onTabChange={(k) => setDetailTab(k as DetailTab)} actions={<button className="btn btn-danger btn-sm" onClick={() => setShowDelete(true)}>{t("common.delete")}</button>}>
        {detailTab === "detail" ? (
          <table className="settings-table" style={{ width: "100%" }}><tbody>
            ...
            <tr><td style={{ fontWeight: 600 }}>Zone ID</td><td className="cell-mono">{selectedItem.id}</td></tr>
            <tr><td style={{ fontWeight: 600 }}>Records</td><td>{String(selectedItem.resourcerecordsetcount)}</td></tr>
            {selectedItem.config?.comment && <tr><td style={{ fontWeight: 600 }}>Comment</td><td>{selectedItem.config.comment}</td></tr>}
            {selectedItem.config && <tr><td style={{ fontWeight: 600 }}>Private</td><td>{selectedItem.config.privatezone ? "Yes" : "No"}</td></tr>}
          </tbody></table>
    ...
    <ServicePageLayout icon="🌐" title={t("services.route53.title")} isLoading={isLoading} error={error} count={items.length} countLabel={t("services.route53.countLabel")} actions={<>
      ...
      {items.length > 0 ? (
        <Splitter direction="horizontal" initialSize={240} minSize={80} maxSize={600} storageKey="vs-split-route53">
          <div style={{ flex: 1, minHeight: 0, overflow: "auto" }}><DataTable columns={[checkboxColumn<HostedZone>(selectedIds, toggle, () => toggleAll_(allIds), allIds, t, (row) => row.id), ...columns]} data={items} getRowId={(row) => row.id} onRowClick={handleRowClick} selectedId={selectedItem?.id} /></div>
          {renderDetailPanel()}
        </Splitter>
