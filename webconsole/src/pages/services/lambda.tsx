...
import { TagSection, useTags, type TagPair } from "@/components/shared/tag-section";
  ...
  const { tags: itemTags, isLoading: tagsLoading, addTags, removeTag, isPending: tagsPending } = useTags(
    {
      ...
      fetchTags: async (arn: string) => {
        const res = await client.listTags({ resource: arn });
        return Object.entries(res.tags ?? {}).map(([key, value]) => ({ key, value }));
      },
      tagResource: async (arn: string, tags: TagPair[]) => {
        const m: Record<string, string> = {};
        for (const t of tags) m[t.key] = t.value;
        await client.tagResource({ resource: arn, tags: m });
      },
      untagResource: async (arn: string, tagKeys: string[]) => {
        await client.untagResource({ resource: arn, tagkeys: tagKeys });
      },
    },
    selectedItem?.functionarn || undefined,
  );
      ...
      <DetailPanel title={selectedItem.functionname} titleIcon="⚡" tabs={[{ key: "detail", label: t("common.tabDetail") }, { key: "json", label: t("common.rawJson") }]} activeTab={detailTab} onTabChange={(k) => setDetailTab(k as DetailTab)} actions={<button className="btn btn-danger btn-sm" onClick={() => setShowDelete(true)}>{t("common.delete")}</button>}>
        {detailTab === "detail" ? (
          <><table className="settings-table"><tbody>
            <tr><td className="detail-label-fixed">{t("common.fields.function")}</td><td className="cell-mono">{selectedItem.functionname}</td></tr>
            <tr><td className="detail-label">{t("common.fields.runtime")}</td><td><span className="badge">{rt}</span></td></tr>
            <tr><td className="detail-label">{t("common.fields.handler")}</td><td className="cell-mono">{selectedItem.handler || "\u2014"}</td></tr>
            <tr><td className="detail-label">{t("common.fields.memory")}</td><td>{selectedItem.memorysize} MB</td></tr>
            <tr><td className="detail-label">{t("common.fields.timeout")}</td><td>{selectedItem.timeout}s</td></tr>
            <tr><td className="detail-label">{t("common.fields.state")}</td><td><span className="badge">{selectedItem.state || "\u2014"}</span></td></tr>
            <tr><td className="detail-label">{t("common.fields.codeSize")}</td><td>{selectedItem.codesize ? `${selectedItem.codesize} bytes` : "\u2014"}</td></tr>
            {selectedItem.description && <tr><td className="detail-label">{t("common.fields.description")}</td><td>{selectedItem.description}</td></tr>}
            {selectedItem.functionarn && <tr><td className="detail-label">{t("common.fields.arn")}</td><td className="cell-mono cell-long">{selectedItem.functionarn}</td></tr>}
            {selectedItem.lastmodified && <tr><td className="detail-label">{t("common.fields.modified")}</td><td>{fmtDate(selectedItem.lastmodified, i18n.language)}</td></tr>}
          </tbody></table>
