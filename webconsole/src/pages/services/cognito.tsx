      ...
      <DetailPanel title={selectedItem.name} titleIcon="🔐" tabs={[{ key: "detail", label: t("common.tabDetail") }, { key: "json", label: t("common.rawJson") }]} activeTab={detailTab} onTabChange={(k) => setDetailTab(k as DetailTab)} actions={<button className="btn btn-danger btn-sm" onClick={() => setShowDelete(true)}>{t("common.delete")}</button>}>
        {detailTab === "detail" ? (
          <table className="settings-table"><tbody>
            ...
            <tr><td className="detail-label">{t("common.fields.poolId")}</td><td className="cell-mono">{selectedItem.id}</td></tr>
            <tr><td className="detail-label">{t("common.fields.status")}</td><td><span className="badge">{selectedItem.status}</span></td></tr>
            {selectedItem.creationdate && <tr><td className="detail-label">{t("common.fields.created")}</td><td>{fmtDate(selectedItem.creationdate, i18n.language)}</td></tr>}
            {selectedItem.lastmodifieddate && <tr><td className="detail-label">{t("common.fields.lastModified")}</td><td>{fmtDate(selectedItem.lastmodifieddate, i18n.language)}</td></tr>}
          </tbody></table>
