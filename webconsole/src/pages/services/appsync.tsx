      ...
      <DetailPanel title={selectedItem.name} titleIcon="🔮" tabs={[{ key: "detail", label: t("common.tabDetail") }, { key: "json", label: t("common.rawJson") }]} activeTab={detailTab} onTabChange={(k) => setDetailTab(k as DetailTab)} actions={<button className="btn btn-danger btn-sm" onClick={() => setShowDelete(true)}>{t("common.delete")}</button>}>
        {detailTab === "detail" ? (
          <><table className="settings-table"><tbody>
            ...
            <tr><td className="detail-label">{t("common.fields.apiId")}</td><td className="cell-mono">{selectedItem.apiid}</td></tr>
            <tr><td className="detail-label">{t("common.fields.authType")}</td><td><span className="badge">{authLabel}</span></td></tr>
            {selectedItem.uris && Object.keys(selectedItem.uris).length > 0 && <tr><td className="detail-label">{t("common.fields.uris")}</td><td className="cell-mono cell-long">{Object.entries(selectedItem.uris).map(([k, v]) => `${k}: ${v}`).join(", ")}</td></tr>}
          </tbody></table>
