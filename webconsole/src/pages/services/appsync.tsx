/**
 * AppSync service page. Lists GraphQL APIs via ListGraphqlApis RPC.
 */
import { useState } from "react";
import { useTranslation } from "react-i18next";
import { useQuery } from "@tanstack/react-query";
import type { ColumnDef } from "@tanstack/react-table";
import { createClient } from "@connectrpc/connect";
import { AppSyncService, type GraphqlApi } from "@/gen/appsync_pb";
import { transport } from "@/lib/transport";
import { useListKey, dropEmpty } from "@/lib/use-service-list";
import { DataTable } from "@/components/shared/data-table";
import { JsonViewer } from "@/components/shared/json-viewer";

const columns: ColumnDef<GraphqlApi, any>[] = [
  {
    accessorKey: "name",
    header: "API Name",
    cell: ({ getValue }) => (
      <span className="cell-mono">{getValue() as string}</span>
    ),
  },
  {
    accessorKey: "apiid",
    header: "API ID",
    cell: ({ getValue }) => (
      <span className="cell-mono">{getValue() as string}</span>
    ),
    size: 100,
  },
  {
    accessorKey: "authenticationtype",
    header: "Auth",
    size: 100,
  },
  {
    accessorKey: "arn",
    header: "ARN",
    cell: ({ getValue }) => (
      <span className="cell-mono" style={{ fontSize: "0.85em" }}>{getValue() as string}</span>
    ),
  },
];

export function AppSyncPage() {
  const { t } = useTranslation();
  const [selectedItem, setSelectedItem] = useState<GraphqlApi | null>(null);

  const client = createClient(AppSyncService, transport);
  const { queryKey } = useListKey("appsync");

  const { data, isLoading, error } = useQuery({
    queryKey,
    queryFn: () => client.listGraphqlApis({}),
    refetchInterval: 30_000,
  });

  const items: GraphqlApi[] = dropEmpty(data?.graphqlapis ?? [], "name");

  if (isLoading) {
    return (
      <div className="content-area">
        <div className="page-header">
          <span className="page-icon">⚡</span>
          <h1>AppSync</h1>
        </div>
        <div className="loading-state">{t("common.loading")}</div>
      </div>
    );
  }

  if (error) {
    return (
      <div className="content-area">
        <div className="page-header">
          <span className="page-icon">⚡</span>
          <h1>AppSync</h1>
        </div>
        <div className="error-state">{t("common.failedToLoad", { error: String(error) })}</div>
      </div>
    );
  }

  return (
    <div className="content-area">
      <div className="page-header">
        <span className="page-icon">⚡</span>
        <h1>AppSync</h1>
        <span className="resource-count">{items.length} APIs</span>
      </div>

      <div className="split-pane">
        <div className="split-table">
          <DataTable
            columns={columns}
            data={items}
            getRowId={(row) => row.apiid}
            onRowClick={setSelectedItem}
            selectedId={selectedItem?.apiid}
          />
        </div>
        {selectedItem && (
          <div className="split-detail">
            <div className="detail-header">
              <h2>{selectedItem.name}</h2>
              <button className="detail-close" onClick={() => setSelectedItem(null)}>
                ✕
              </button>
            </div>
            <JsonViewer data={selectedItem} />
          </div>
        )}
      </div>
    </div>
  );
}
