      ...
      <DetailPanel title={selectedItem.streamname} titleIcon="🌊" tabs={[{ key: "detail", label: t("common.tabDetail") }, { key: "json", label: t("common.rawJson") }]} activeTab={detailTab} onTabChange={(k) => setDetailTab(k as DetailTab)} actions={<button className="btn btn-danger btn-sm" onClick={() => setShowDelete(true)}>{t("common.delete")}</button>}>
        {detailTab === "detail" ? (
          <table className="settings-table"><tbody>
            <tr><td className="detail-label-fixed">Name</td><td className="cell-mono">{selectedItem.streamname}</td></tr>
            <tr><td className="detail-label">Status</td><td><span className="badge">{String(selectedItem.streamstatus)}</span></td></tr>
            <tr><td className="detail-label">ARN</td><td className="cell-mono cell-long">{selectedItem.streamarn}</td></tr>
            {selectedItem.streamcreationtimestamp && <tr><td className="detail-label">Created</td><td>{fmtDate(selectedItem.streamcreationtimestamp, i18n.language)}</td></tr>}
          </tbody></table>
