      ...
      <DetailPanel title={selectedItem.databasename} titleIcon="⏱" tabs={[{ key: "detail", label: t("common.tabDetail") }, { key: "json", label: t("common.rawJson") }]} activeTab={detailTab} onTabChange={(k) => setDetailTab(k as DetailTab)} actions={<button className="btn btn-danger btn-sm" onClick={() => setShowDelete(true)}>{t("common.delete")}</button>}>
        {detailTab === "detail" ? (
          <table className="settings-table"><tbody>
            <tr><td className="detail-label-fixed">{t("common.fields.database")}</td><td className="cell-mono">{selectedItem.databasename}</td></tr>
            <tr><td className="detail-label">{t("common.fields.tables")}</td><td>{selectedItem.tablecount}</td></tr>
            {selectedItem.creationtime && <tr><td className="detail-label">{t("common.fields.created")}</td><td>{fmtDate(selectedItem.creationtime, i18n.language)}</td></tr>}
            {selectedItem.lastupdatedtime && <tr><td className="detail-label">{t("common.fields.updated")}</td><td>{fmtDate(selectedItem.lastupdatedtime, i18n.language)}</td></tr>}
