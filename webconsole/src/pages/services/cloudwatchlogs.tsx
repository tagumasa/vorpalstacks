...
export function CloudWatchLogsPage() {
  ...
  const { tags: itemTags, isLoading: tagsLoading, addTags, removeTag, isPending: tagsPending } = useTags(
    ...
    selectedItem?.loggrouparn || undefined,
  );
      ...
      <DetailPanel title={selectedItem.loggroupname} titleIcon="📜" tabs={[{ key: "detail", label: t("common.tabDetail") }, { key: "json", label: t("common.rawJson") }]} activeTab={detailTab} onTabChange={(k) => setDetailTab(k as DetailTab)} actions={<button className="btn btn-danger btn-sm" onClick={() => setShowDelete(true)}>{t("common.delete")}</button>}>
        {detailTab === "detail" ? (
          <><table className="settings-table"><tbody>
            <tr><td className="detail-label-fixed">{t("common.fields.name")}</td><td className="cell-mono">{selectedItem.loggroupname}</td></tr>
            <tr><td className="detail-label">{t("common.fields.class")}</td><td>{selectedItem.loggroupclass || "\u2014"}</td></tr>
            <tr><td className="detail-label">{t("common.fields.arn")}</td><td className="cell-mono cell-long">{selectedItem.loggrouparn}</td></tr>
          </tbody></table>
